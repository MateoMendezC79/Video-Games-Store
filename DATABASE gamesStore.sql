...
WHERE cliente.id_cliente = @id_cliente AND asesor.id_asesor=cliente.id_asesor 
END
...
WHERE producto.id_producto = @id_producto AND producto.id_producto=cliente_producto.id_producto AND cliente.id_cliente=cliente_producto.id_cliente
...
CREATE PROCEDURE actualizar_edad (@id_asesor INT,@edad INT)
AS
begin	
UPDATE asesor SET asesor.edad = @edad
WHERE asesor.id_asesor = @id_asesor
END
